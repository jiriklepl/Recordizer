...
public:
  ...
  NoteEvent on_event(Duration when) const {
    return {when, note(), on_velocity(), NoteEvent::EventType::ON};
  }
  NoteEvent off_event(Duration when) const {
    return {when + duration(), note(), off_velocity(),
